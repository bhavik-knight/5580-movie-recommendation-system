...
from src.config import (
    FILTERED_MOVIE_IDS_FILE,
    ITEM_SIMILARITY_FILE,
    MOVIE_LOOKUP_FILE,
    RATINGS_MATRIX_NORM_FILE,
    U_ITEM_NAMES,
    U_ITEM_PATH,
)


def load_inputs() -> tuple[pd.DataFrame, Any, pd.DataFrame]:
    """
    ...
    normalized_matrix = normalized_matrix.rename(columns=int)
...
def compute_similarity(normalized_matrix: pd.DataFrame) -> pd.DataFrame:
    ...
    similarity_df = pd.DataFrame(similarity_array, index=movie_ids, columns=movie_ids)
...
def run_sanity_checks(similarity_df: pd.DataFrame, lookup_df: pd.DataFrame) -> None:
    ...
    def get_top_similar(movie_title: str, top_n: int = 5) -> str:
        ...
        sim_scores: pd.Series = similarity_df[movie_id].drop(movie_id)

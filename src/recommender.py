...
def load_data() -> tuple[pd.DataFrame, dict[str, int], dict[int, str], pd.DataFrame]:
    ...
    similarity_df = similarity_df.rename(columns=int)
...
def recommend(input_titles: list[str], top_n: int = 10, min_score: float = 0.05) -> list[dict]:
    ...
    for rank, (movie_id, score) in enumerate(top_scores.items(), start=1):
        ...
        movie_title = id_to_title.get(int(str(movie_id)), "Unknown Title")
        ...
        recommendations.append({"rank": rank, "title": movie_title, "score": score, "reason": reason})
...
def main() -> None:
    ...
    print("Evaluation results saved to: output/recommendation_test_results.txt")
